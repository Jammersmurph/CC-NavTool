...
local injected = table.concat({
  ...
  '',
  '-- Status is dashboard-only. Remove desktop polling and its refresh shortcut.',
  'source = select(1, replacePlainOnce(source, "local refreshTimer=os.startTimer(1)\\n", ""))',
  ...
  'source = select(1, replacePlainOnce(source, timerBranch, [=[  if event=="key" then]=]))',
  ...
  'local keyAnchor = [[  if event=="key" then',
  '    local key=a]]',
  'local mouseBranch = [[  if event=="mouse_click" then',
