...
local injected = table.concat({
  ...
  '    if data.activeRoute or data.activeSchedule or data.preferences.monitorTelemetry==true then local status=refresh(data,true); advanceAutomation(data,status) end',
  '    refreshTimer=os.startTimer(1)',
  '  elseif event=="key" then]=]',
  ...
  '    writeAt(3,7,"4. Front calibration",colors.cyan); writeAt(25,7,"open")',
  ...
  '      elseif a==keys.four then action="front"',
  ...
  '    elseif action=="monitor" then data.preferences.monitorTelemetry=not (data.preferences.monitorTelemetry==true); config.monitorTelemetry=data.preferences.monitorTelemetry==true; saveConfig(); if data.preferences.monitorTelemetry then if renderMonitorTelemetry then renderMonitorTelemetry(data) end; if refresh then refresh(data,true) end elseif clearLocalMonitors then clearLocalMonitors() end',
  '    elseif action=="front" and frontCalibrationPage then frontCalibrationPage(data)',
